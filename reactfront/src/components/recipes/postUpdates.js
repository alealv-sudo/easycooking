...
const UpdateRecipes = () => {
  ...
  const deleteIngredients = (id_recipe, ingredients) => {
    axios
      .delete(process.env.REACT_APP_API_URL + "ingredients/" + id_recipe)
      .then(function response(response) {
  ...
  const onFinish = (values) => {
    if (fileList.length !== 0) {
      ...
      axios
        .put(process.env.REACT_APP_API_URL + "post/", recipes)
        .then(function response(response) {
  ...
  const deleteImage = (image_recipe_id) => {
    axios
      .delete(
        process.env.REACT_APP_API_URL + "google/delete/" + image_recipe_id
      )
      .then((response) => {})
      .catch((error) => {});
  };
