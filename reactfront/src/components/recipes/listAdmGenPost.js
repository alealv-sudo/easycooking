...
import { List, Space, Button, Popconfirm, Image, Spin } from "antd";
...
export default function ListDay({ userId }) {
  ...
  async function getListCalendar() {
    axios
      .get(process.env.REACT_APP_API_URL + "GeneralPost/user/" + userId)
      .then((response) => {
              ...
              <div className="button-table-list">
                <div>
                  <Popconfirm
                    ...
                    cancelText="No"
                    onConfirm={() =>
                      navigate("/private/editGPost/" + item.idRecipe)
                    }
                  >
                    <Button type="default">
                      <EditOutlined />
