...
export default function Profile() {
    ...
    const onFinish = (values) => {
        ...
        axios.put(process.env.REACT_APP_API_URL + 'profile/', profileTemp)
            .then((response) => {
                if ((fileList.length !== 0 && isUpProfile) || (fileListBG.length !==0 && isUpBackground)) {
    ...
    const propsBG = {
        ...
        beforeUpload: (file) => {
            ...
            // Check that the file is pdf
            const isFile = file.type === 'image/png'
                || file.type === 'image/jpg'
                || file.type === 'image/jpeg'
                ;
            // If it isn't pdf then delete the file
    ...
    const handleUpload = (profile_id) => {
        ...
        if (isUpProfile && isUpBackground) {
            ...
            .then(resPF => {
                    ...
                    axios.post(process.env.REACT_APP_API_URL + "google/upload/", formDataBG)
                    .then(resBG => {
            ...
            axios.post(process.env.REACT_APP_API_URL + "google/upload/", formPG)
                .then(resPG => {
                    if (isUpProfile) {
    ...
    const updateIMG = (profile_id, img_id ,background_id) => {
        ...
        axios.put(process.env.REACT_APP_API_URL + 'profile/', ImageProfile)
