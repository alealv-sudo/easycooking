...
FavoriteRecipeCTRL.handleLikeClicked = async (req, res) => {
  try {
    ...
    if (existingFavorite) {
      ...
      const maxId = await FavoriteRecipeModel.max("id");
      const newId = maxId ? maxId + 1 : 1; // If there are no entries yet, start from 1
...
FavoriteRecipeCTRL.handleFavClicked = async (req, res) => {
  try {
    const { userId, postId } = req.body;
    ...
    if (existingFavorite) {
      ...
      const newId = maxId ? maxId + 1 : 1; // If there are no entries yet, start from 1
      // If the favorite doesn't exist, create it and increment the likes count
      await FavoriteRecipeModel.create({
        id: newId,
        userId: userId,
        recipeId: postId,
      });
