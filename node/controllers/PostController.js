...
/* Post paginados para el selector de tipo de receta */

PostCTRL.getSelectPostsPaginated = async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 12;
    const userId = req.query.userId; // Get the userId from query parameters
    const typeRecipe = req.query.typeRecipe;
...
PostCTRL.getPostSimilar = async (req, res) => {
  ...
  try {
    ...
    const totalPosts = await PostModel.count({
      where: {
        recipe_name: { [Op.iLike]: valueStyle },
      },
    });
      ...
      include: [
        {
          ...
          where: {
            ingredient: {
              [Op.or]: valueStyle.map((value) => ({ [Op.iLike]: value })),
            },
          },
          attributes: { exclude: ["id", "recipeId"] },
        },
